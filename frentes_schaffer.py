...
if __name__ == "__main__":
    print("Fast non-dominated sort - funcion de Schaffer")
    ...
    for i in range(len(frentes)):
        ...
        ax.scatter(
            ...
            color=colores[i], s=25,
            label=etiqueta,
        )

    ax.set_xlabel("f1(x) = x^2")
    ax.set_ylabel("f2(x) = (x-2)^2")
    ax.set_title(f"Frentes de no dominancia - Schaffer ({len(frentes)} frentes)")
    ax.grid(True, alpha=0.2)
    ax.legend(frameon=False, ncol=2, fontsize=7, title=f"Primeros {max_leyenda} frentes")
    fig.tight_layout()
    fig.savefig("frentes_schaffer.png", dpi=150)
    plt.show()
